...
class SymbolExtractor:
    ...
    def _get_symbol_name(self, node: tree_sitter.Node, source_code: str) -> Optional[str]:
    ...
    def _extract_from_node(
        ...
        if symbol_type:
            ...
            if name:
                ...
                signature = extract_signature(self._get_node_text(node, source_code), symbol_type, "csharp")
        ...
        for child in node.children:
            self._extract_from_node(child, source_code, file_path, project_id, symbols, next_parent)
    ...
    def _extract_from_node(
        ...
        if symbol_type:
            ...
            if name:
                ...
                line_start, line_end = self._get_line_range(node)
                signature = extract_signature(self._get_node_text(node, source_code), symbol_type, "java")
                raw_code = self._get_node_text(node, source_code)
                
                # Check if this is a constant
                if symbol_type == SymbolType.FIELD and ("static" in raw_code.lower() and "final" in raw_code.lower()):
                    symbol_type = SymbolType.CONSTANT
    ...
    def _extract_from_node(
        ...
        if symbol_type:
            ...
            if name:
                ...
                line_start, line_end = self._get_line_range(node)
                signature = extract_signature(self._get_node_text(node, source_code), symbol_type, "python")
                raw_code = self._get_node_text(node, source_code)
                
                # Check if this is a constant
                if symbol_type == SymbolType.VARIABLE and is_constant_symbol(name, "python"):
                    symbol_type = SymbolType.CONSTANT
    ...
    def _extract_from_node(
        ...
        if symbol_type:
            ...
            if name:
                ...
                line_start, line_end = self._get_line_range(node)
                signature = extract_signature(self._get_node_text(node, source_code), symbol_type, "javascript")
                raw_code = self._get_node_text(node, source_code)
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                # Extract characters
                    ...
                    char_match = re.match(r'define\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*Character', line)
                    if char_match:
