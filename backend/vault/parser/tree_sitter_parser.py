...
from vault.storage.models import Project, Symbol, IndexStatus
...
class TreeSitterParser:
    ...
    def __init__(self, project_repo: ProjectRepository, symbol_repo: SymbolRepository) -> None:
    ...
    async def parse_multiple_projects(self, project_ids: List[UUID]) -> Dict:
        ...
        for project_id in project_ids:
            if project_id not in self._active_tasks:
                task = asyncio.create_task(self.parser.parse_project(project_id))
                self._active_tasks[project_id] = task
                task.add_done_callback(lambda t, pid=project_id: self._active_tasks.pop(pid, None))
                tasks.append(task)
