...
from vault.storage.models import Project, IndexStatus, ProjectType
...
class ProjectScanner:
    ...
    async def scan_root_directories(self, root_paths: Optional[List[str]] = None) -> List[Project]:
    ...
    async def _create_or_update_project(self, directory: Path) -> Optional[Project]:
        ...
        try:
            ...
            existing_project = await self.project_repo.get_by_path(str(directory.absolute()))
...
class ProjectDiscoveryService:
    ...
    async def get_project_statistics(self) -> dict:
        ...
        for project in all_projects:
            # Count by type
            project_type = project.type.value if hasattr(project.type, "value") else project.type
            stats["by_type"][project_type] = stats["by_type"].get(project_type, 0) + 1
            
            # Count by status
            status = project.index_status.value if hasattr(project.index_status, "value") else project.index_status
